...
posts = Post.create([{ author: users[0], title: 'Keys to a Successful Presidency', content: 'A good president needs to have a dog named Bo.' },
                     { author: users[1], title: 'How to Enjoy Retirement', content: 'Painting is a really good way to stay busy in retirement.' }])
...
comments = Comment.create([{ post: posts[0], user: users[2], body: 'In my experience, a cat named Socks is vital.' },
                           { post: posts[1], user: users[3], body: "I've found peanut farming to be a really fun hobby." },
                           { post: posts[1], user: users[0], body: 'Who here likes to body surf?' }])

Comment.create({ post: posts[0], user: users[1], body: 'What about a dog named Miss Beazley?', parent: comments[0] })
